...
2. `pnpm build` runs `vite build`, which **prerenders every route**. A route that throws while rendering fails the build. This is the real gate for the new page and the new component.
...
```sh
cd site && pnpm lint && pnpm build && pnpm check:prose
```
...
**Known ordering hazard.** Task 3 adds a link to `/docs/setup` before Task 6 creates the route. This is deliberate and safe: it is a plain anchor, so it cannot fail a build, and both land before the branch is pushed in Task 9. If Tasks 3 through 8 are ever reordered, Task 6 must still precede Task 9.
