...
One thing bounds it, and it is not a fix: it only works during a pairing window
*you* opened, so a relay cannot enrol itself unprompted. The device it enrols
...
Terminal content is not something it declines to store; it is something the
bytes crossing it cannot be turned into.

The qualifier from the first answer stays attached to that sentence, because it
is the same trust boundary and it does not stop being true here: the guarantee
is about what crosses the relay, not about the JavaScript the origin serves you.
A hosted relay that shipped a modified bundle would be reading your terminal
through a device it paired, not through the ciphertext it carries. Which is why
self-hosting comes first in this document and why a hosted relay is one option
among several rather than the way flue works.
