...
// TestSaveRelayRoundTripsAHostedRelay: the same file carries both shapes, and
// which fields are set is what says which one it is. A machine linked to
// flue.sh has no shared secret at all — the relay verifies a token this daemon
// mints from the control plane — so `secret` is absent and the three SaaS
// fields are present.
func TestSaveRelayRoundTripsAHostedRelay(t *testing.T) {
	base := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", base)
	...
	want := Relay{
		...
		EnrollmentToken: "enrollment-token-for-this-machine",
	}
	if err := SaveRelay(want); err != nil {
		t.Fatalf("SaveRelay: %v", err)
	}
	...
	got, ok, err := LoadRelay()
	if err != nil || !ok {
		t.Fatalf("LoadRelay = (%v, %v)", ok, err)
	}
	if got != want {
		t.Fatalf("LoadRelay = %+v, want %+v", got, want)
	}
	if !got.Hosted() {
		t.Error("Hosted() = false for a relay.json with an enrollment token")
	}
	if got.Secret != "" {
		t.Error("a hosted relay was saved with a shared secret")
	}
}

// TestSelfHostedRelayIsNotHosted: the two are told apart by the enrollment
// token and nothing else, because that is the field that decides what the dial
// presents. A self-hosted relay.json written before this field existed must
// keep working.
func TestSelfHostedRelayIsNotHosted(t *testing.T) {
	t.Parallel()
	r := Relay{URL: "wss://r.example/daemon", Secret: "s", Origin: "https://r.example"}
	if r.Hosted() {
		t.Error("Hosted() = true for a relay with a shared secret and no enrollment token")
	}
}

// TestSaveRelayOmitsTheFieldsThatDoNotApply: relay.json is a file people open
// and edit by hand — it is how a self-hosted relay is configured — and a
// self-hosted one listing three empty flue.sh fields reads like a
// half-finished link. Both shapes write only their own.
func TestSaveRelayOmitsTheFieldsThatDoNotApply(t *testing.T) {
	base := t.TempDir()
	t.Setenv("XDG_CONFIG_HOME", base)
	...
	b, err := os.ReadFile(filepath.Join(base, "flue", "relay.json"))
	if err != nil {
		t.Fatalf("read relay.json: %v", err)
	}
	for _, absent := range []string{"control_plane", "device_id", "enrollment_token"} {
		if strings.Contains(string(b), absent) {
			t.Errorf("a self-hosted relay.json mentions %q:\n%s", absent, b)
		}
	}

	if err := SaveRelay(Relay{
		URL:             "wss://relay.flue.sh/daemon",
		Origin:          "https://relay.flue.sh",
		ControlPlane:    "https://app.flue.sh",
		DeviceID:        "abc123abc123",
	...
	if strings.Contains(string(b), `"secret"`) {
		t.Errorf("a hosted relay.json carries a secret field:\n%s", b)
	}
}
