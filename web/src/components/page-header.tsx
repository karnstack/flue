 ...
 * The shell's own bar keeps what belongs to the shell: the sidebar trigger and
 * the wordmark.
 */
      ...
      <div className="flex items-start justify-between gap-x-4">
        <nav aria-label="Breadcrumb" className="min-w-0">
          <ol className="flex flex-wrap items-baseline gap-x-2">
            {crumbs.map((crumb, i) => (
              // Index in the key because a trail is positional: two crumbs may
              // legitimately read the same (a session named after its machine)
              // and only their depth tells them apart.
              <li key={`${i}:${crumb.label}`} className="flex items-baseline gap-x-2">
                ...
                {i === current ? (
                  ...
                  /*
                    A router Link, never a plain anchor: a page load would tear
                    down the tab's one socket, and the trail is chrome — the
                    cheapest way there is to leave a screen.

                    Note what `to` does *not* buy here. TanStack rejects a path
                    literal the route tree has no match for, but a value it
                    cannot see the literal of — this one — infers as `string`
                    and is waved through, so a crumb pointing nowhere is a
                    runtime not-found rather than a compile error. Callers that
                    can write the literal should; router.test.tsx is what holds
                    the line for the paths the app itself links to.
                  */
                  <Link
                    to={crumb.to}
